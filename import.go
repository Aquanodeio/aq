...
type importOptions struct {
	...
	name     string
	yes      bool
	// resumeVolumeID, when set, skips survey/confirm/start entirely and
	// resumes a previously started (but not completed) import for this
	// volume id: re-mint credentials, re-run capture into the SAME
...
// anywhere that isn't Aquanode), captures its /workspace-equivalent data into
// a new Aquanode Volume (D12: this used to create a whole Setup with a
// synthesized recipe; now it adopts the captured prefix as a Volume, the same
// three-noun split every other pod/environment/volume surface uses). It is
// survey-first: nothing is captured or uploaded until the user has seen
// exactly what will and won't be, and confirmed.
//
// There is no more --launch here: a bare Volume carries no installed-package
// manifest or recipe to provision hardware from, so "rent a GPU and restore
// onto it" now means creating a pod (any built-in Environment, this Volume
// attached) from the console, not a flag on this command.
func importCmd(args []string) error {
	...
	name := fs.String("name", "", "Name the resulting volume (default: derived from the box's hostname)")
	yes := fs.Bool("yes", false, "Skip the interactive confirmation")
	resume := fs.String("resume", "", "Resume a previously started import for this volume id (re-mints credentials; restic dedups what already landed)")
	...
	return runImport(importOptions{
		...
		yes:            *yes,
		resumeVolumeID: strings.TrimSpace(*resume),
		out:            os.Stdout,
		errOut:         os.Stderr,
	})
}

// runImport drives the whole flow: survey → confirm → create the volume →
// capture + upload → register the first history point.
func runImport(opts importOptions) error {
	...
	fmt.Fprintf(opts.out, "\nCapturing and uploading to volume %s...\n", start.VolumeID)
	...
	// storagePrefix is what makes a volume's repo unique, and ResticBackupID
	// is the fixed trailing path segment (currently the literal "repo") for
	// every portable volume. An earlier version of this command used the
	...
	if start.ResticBackupID == "" {
		return fmt.Errorf("volume %s was created but the orchestrator did not return a restic_backup_id, refusing to guess where in storage to write the capture (see CONTRACT.md section G); update aq or the orchestrator, then resume with `aq import --resume %s`", start.VolumeID, start.VolumeID)
	}
	...
	if err != nil {
		return fmt.Errorf("volume %s exists but the capture failed, resume it with `aq import --resume %s` once fixed (restic dedups what already landed): %w", start.VolumeID, start.VolumeID, err)
	}

	// 4. Register the capture as the volume's first history point. The
	// observation is forwarded exactly as ogre emitted it (recorded for
	// reference; a Volume's manifest was never restorable to begin with).
	...
	if err != nil {
		return fmt.Errorf("capture succeeded but registering the volume failed, resume with `aq import --resume %s` to retry: %w", start.VolumeID, err)
	}

	fmt.Fprintf(opts.out, "\n✓ Imported into volume %s. See it with `aq volume ls`.\n", complete.VolumeID)
	fmt.Fprintln(opts.out, "Attach it to a pod (any built-in environment, this volume) from the console to bring it online.")
...
// user sees what turning this box's data into a durable Volume will cost
// before a single byte is captured or a Volume row created.
func printHeldStorageCost(out io.Writer, obs api.ImportObservation) {
	...
	fmt.Fprintf(out, "\nOnce imported, this volume's held storage (%s%s) is billed at %s.\n", floor, formatBytes(total), heldStorageRateLabel)
}

// printImportWarnings prints /volumes/import/complete's non-blocking
// warnings, if any.
...
// restic password decrypting the whole volume, left behind by an abandoned
...
// parallel-billing volume.
func runImportResume(client *api.Client, opts importOptions) error {
	out := opts.out

	fmt.Fprintf(out, "Resuming import for volume %s...\n", opts.resumeVolumeID)

	refreshed, err := client.RefreshImportCredentials(opts.resumeVolumeID)
	if err != nil {
		return fmt.Errorf("could not resume import for volume %s: %w", opts.resumeVolumeID, err)
	}
	if refreshed.ResticBackupID == "" {
		return fmt.Errorf("volume %s: the orchestrator did not return a restic_backup_id, refusing to guess where in storage to write the capture (see CONTRACT.md section G)", opts.resumeVolumeID)
	}
	...
	if err != nil {
		return fmt.Errorf("resume capture failed; re-run `aq import --resume %s` again once fixed: %w", opts.resumeVolumeID, err)
	}
	...
	if err != nil {
		return fmt.Errorf("resume capture succeeded but registering the volume failed: %w", err)
	}

	fmt.Fprintf(out, "\n✓ Resumed import into volume %s. See it with `aq volume ls`.\n", complete.VolumeID)
	fmt.Fprintln(out, "Attach it to a pod (any built-in environment, this volume) from the console to bring it online.")
